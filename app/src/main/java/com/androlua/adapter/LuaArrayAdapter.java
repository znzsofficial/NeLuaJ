  ...
  private void setHelper(View view, Object value) {
    if (view instanceof TextView) {
      ...
      try {
        ...
        else if (value instanceof Number) drawable = mRes.getDrawable(((Number) value).intValue());
        ...
        if (drawable instanceof BitmapDrawable) {
          ...
          int h = w / 4;
          img.setLayoutParams(new ViewGroup.LayoutParams(w, h));
        } else if (drawable instanceof LuaBitmapDrawable) {
          LuaBitmapDrawable dw = (LuaBitmapDrawable) drawable;
          int w = dw.getWidth();
          int h = dw.getHeight();
          if (w < 0 || h < 0) {
            w = mContext.getWidth();
