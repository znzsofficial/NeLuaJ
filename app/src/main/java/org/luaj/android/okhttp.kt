...
/** General asynchronous HTTP API exposed to Lua scripts. */
class AsyncOkHttp(
    ...
    private fun parseBody(body: String) = FormBody.Builder().apply {
        for (item in body.split("&")) {
            val pair = item.split("=", limit = 2)
            add(pair[0], pair.getOrElse(1) { "" })
        }
    }.build()
        ...
        enqueue(object : okhttp3.Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                try {
                    ...
                    context.runOnUiThread {
                        runCatching {
                            callback.call(code.toLuaValue(), body.toLuaValue(), response.toLuaInstance())
                        }.onFailure { context.sendMsg("回调发生异常：${it.message}") }
                    }
                } catch (error: Exception) {
                    context.sendMsg("网络请求发生异常：${error.message}")
                }
            }
        })
}
