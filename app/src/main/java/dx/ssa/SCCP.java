...
public class SCCP {
    ...
    private void addUsersToWorklist(int reg, int latticeValue) {
        if (latticeValue == VARYING) {
            varyingWorklist.addAll(ssaMeth.getUseListForRegister(reg));
        } else {
            ssaWorklist.addAll(ssaMeth.getUseListForRegister(reg));
        }
    }
    ...
    private void simulateBranch(SsaInsn insn) {
        ...
        if (opcode.getBranchingness() == Rop.BRANCH_IF) {
            ...
            if (cA != null && sources.size() == 1) {
                // not yet supported
                if (((TypedConstant) cA).getBasicType() == Type.BT_INT) {
                    constantBranch = true;
