...
--- 重开会话时按记录的技能名恢复激活（正文经 SkillManager 重查；
--- 找不到该技能时保持清空）。后续发送仍会按消息重新匹配。
function _M.restoreSkillFromConv(conv)
  activeSkill = nil
  local skills = type(conv) == "table" and conv.skills or nil
  if type(skills) ~= "table" then return nil end
  for name in pairs(skills) do
    local skill = SkillManager.findByName(name)
    if skill then
      activeSkill = skill
      break
    end
...
-- ─── 工具定义（OpenAI function calling 格式，见 mods/agent/ToolSchemas.lua）──

local ToolSchemas = require "mods.agent.ToolSchemas"
_M.TOOLS = ToolSchemas
...
-- ─── 补丁引擎 ──
-- SEARCH/REPLACE 与 Unified Diff 的解析、宽松匹配、应用算法已抽到 mods/agent/PatchEngine.lua（纯算法，含独立单元测试）。
-- 调用点一律走 PatchEngine 导出；对外保留 _M.applyPatch 转发。
...
local function legacyExecuteTool(name, args)
  ...
  elseif name == "apply_patch" then
    ...
    local newContent, countOrErr, locations = PatchEngine.applyPatch(original, patch)
    ...
    local locText = PatchEngine.formatPatchLocations(locations)
  ...
  elseif name == "replace_in_file" then
    ...
    local newContent, replaced = PatchEngine.plainReplace(original, old, new, maxCount)
    if replaced == 0 then
      -- 字面量未命中时回退到忽略空白差异的整行替换
      local okFuzzy, cnt = PatchEngine.fuzzyLineReplace(original, old, new, maxCount)
...
--- 每回合工具轮数上限（含子代理轮次）；0 = 不限制（默认）
function _M.getMaxRounds()
  local v = tonumber(this.getSharedData("ai_max_rounds", "0")) or 0
  if v < 0 then v = 0 end
  return math.floor(v)
end
...
SubagentRunner.configure({
  ...
  getMaxRounds = function() return _M.getMaxRounds() end,
...
_M.applyPatch = PatchEngine.applyPatch
