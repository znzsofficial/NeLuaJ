...
local function persist()
  local fs = cfg()
  if not fs.saveState then return end
  ...
  while true do
    ...
    elseif #redoStack > 0 then
      table.remove(redoStack, 1)
    else
      return
    end
  end
  pcall(fs.saveState, encoded)
end
...
local function snapshot(path, stats)
  ...
  local kind = fs.type(path)
  if kind == "file" then
    stats.files = stats.files + 1
    if stats.files > MAX_SNAPSHOT_FILES then return nil, "文件数量超过快照上限" end
    local content = fs.read(path)
    if content == nil then return { path = path, kind = "missing", fingerprint = "missing" } end
  ...
  elseif kind == "dir" then
    local files = {}
    local dirs = {}
    for _, child in ipairs(fs.listFiles(path) or {}) do
...
function _M.begin(name, args)
  ...
  return { name = name, scope = cfg().scope and cfg().scope(), paths = paths, before = before }
end

function _M.finish(transaction, result)
  loadState()
  if not transaction or not cfg().resultSucceeded(result) then return end
  local stats = { bytes = 0, files = 0 }
  local after = {}
...
local function restoreState(state)
  ...
  if state.kind == "dir" then
    ...
    for _, child in ipairs(fs.listFiles(state.path) or {}) do
      if not fs.remove(child) then return false end
    end
    for _, dir in ipairs(state.dirs or {}) do fs.mkdir(dir) end
...
local function apply(transaction, expectedKey, restoreKey)
  ...
  for i = #expected, 1, -1 do
    if not restoreState(transaction[restoreKey][i]) then
      ...
      for rollbackIndex = #current, 1, -1 do pcall(restoreState, current[rollbackIndex]) end
      return false, "恢复文件失败，已尝试回滚: " .. tostring(transaction[restoreKey][i].path)
    end
  end
  return true
end
