...
NeLuaJ+ 会优先读取全局主题色种子 `theme_seed_color`。如果已设置，调用 `this.dynamicColor()` 的页面会自动按该种子色生成色板；如果未设置，则继续使用系统壁纸动态取色。
...
设置页提供主题色选择器，用户可以直接从预设色板中选择，或进入自定义颜色输入。保存后会把值写入 `theme_seed_color`，随后应用内所有调用 `this.dynamicColor()` / `activity.dynamicColor()` 的 Activity 都会自动使用这个种子色。

当前 `theme_seed_color` 以整数 ARGB 保存，读取时可以直接传给 `this.dynamicColor(seedColor)`；旧版本留下的字符串颜色值也仍然兼容。

```lua
-- 读取当前保存的主题色种子
local seedColor = this.getSharedData("theme_seed_color")
...
> **注意**：如果项目已经保存了 `theme_seed_color`，建议不要在同一个 Activity 里手动再调用一次 `dynamicColor(seedColor)` 叠加其他种子色，以免覆盖用户设置。
